const FormDetails = require('./object/FormDetails');
const formRest = require('../FormRest/index');
...
class FormManager {
    ...
    downloadForms() {
        ...
        return this.dataManager.init().
             ...
             */
            then(({needsUpdating, serverIndex, localIndex}) => {
             ...
             */
            then(({forms, serverIndex, localIndex}) => {
                ...
                return Promise.all(Object.keys(forms).map(name => { // For every downloaded form
                     ...
                     */
                    const formDetails =
                        FormDetails.createFormDetailsFromContent(
                            form.result, name, serverIndex[name].lastModified);

                    // Write them to file
                    console.log(`Writing ${name}`);
                    return this.dataManager.write(name, form.result).
                        then(() => {
    ...
    checkForUpdates() {
        ...
        return this.dataManager.init().then(() => {
            ...
            return new Promise((resolve, reject) => {
                formRest.getFormsIndex().then((serverIndex) => {
                    resolve({
     ...
     */
    getFormsData() {
        return this.dataManager.read('index').then((content) => {
